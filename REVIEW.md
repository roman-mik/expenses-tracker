...
2. ~~**Set up backups.**~~ **Status: workflow authored and restore-drilled locally; production setup still needs three manual steps from you.** `.github/workflows/backup.yml` added (branch `p0-backups`) — daily `supabase db dump` → `gpg` → push to a private `kapa-backups` repo, plus a 90-day artifact as a second copy. Ran an actual restore drill against the local stack rather than assuming it would work: dumped, wiped, and restored the local DB, verified the fixture data round-tripped exactly. **None of the review's predicted failure modes reproduced** — `supabase db dump --data-only` opens with `SET session_replication_role = replica`, which disables every trigger and FK check for the load, so `on_auth_user_created` did not fire and there were no FK-ordering fights; `auth.users` is included in the dump by default. Runbook with the exact verified commands is in `README.md` § Backups & restore.
   - **Still needed from you, outside what I can do**: (1) create the private `roman-mik/kapa-backups` repo; (2) set the three repo secrets (`SUPABASE_DB_URL`, `BACKUP_PASSPHRASE`, `BACKUP_REPO_TOKEN` — setup notes are in the workflow file's header comment); (3) run the workflow once via `workflow_dispatch` against production and do one real restore drill with the actual production dump, not just the local stand-in above.
