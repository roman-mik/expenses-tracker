...
2. ~~**Set up backups.**~~ **Status: fixed and confirmed running against production.** `.github/workflows/backup.yml` added (branch `p0-backups`) — daily `supabase db dump` → `gpg` → push to a private `kapa-backups` repo, plus a 90-day artifact as a second copy. Ran an actual restore drill against the local stack before shipping rather than assuming the commands work: dumped, wiped, and restored the local DB, verified the fixture data round-tripped exactly. **None of the review's predicted failure modes reproduced** — `supabase db dump --data-only` opens with `SET session_replication_role = replica`, which disables every trigger and FK check for the load, so `on_auth_user_created` did not fire and there were no FK-ordering fights; `auth.users` is included in the dump by default. Runbook with the exact verified commands is in `README.md` § Backups & restore. First live run against production hit a connection-string parse error (`SUPABASE_DB_URL` secret needs the dashboard's own copy-pasted, already-percent-encoded connection string — a hand-edited or retyped password segment breaks the parser); fixed by re-copying the URI from Supabase → Project Settings → Database, and the workflow now runs clean.
   - **Still worth doing**: one real restore drill using an actual production dump (not just the local stand-in above), to confirm the same trigger-bypassing behavior holds against the live schema.
3. ~~**Fix `expenses.user_id → auth.users ON DELETE CASCADE`**~~ **Status: fixed** (`0007_expense_attribution.sql`, branch `p0-expense-attribution`, landed together with item 5 below — same migration, same underlying invariant). FK changed to `ON DELETE SET NULL`, `user_id` made nullable, `delete_account()` RPC added as the explicit anonymization path. Verified: deleting a user out of `auth.users` leaves their expenses in the household with `user_id = null`, `household_id` unchanged. New pgTAP suite `expense_attribution.sql` (9 assertions). App-side: `Expense.addedBy` is now `string | null`; `attribution.ts` renders a translated "former member" label for the null case (see item 5's i18n note).
4. ~~**pgTAP RLS suite.**~~ **Status: fixed** (branch `p0-rls-tests`). Added `tests.login_as`/`tests.logout` helpers, retrofitted `join_household.sql` to run as `authenticated` under real RLS, and added `rls.sql` (16 assertions across all 7 tables: cross-household read/write denial, plus attribution spoofing). Verified the acceptance gate by hand: loosening `expenses_select` to `using (true)` turns the suite red; reverting turns it green. Existing assertions did *not* break, for an unexpected reason — see below. *(Security/Testing, Critical)*
   - **Bonus finding surfaced by this work, not in the original review**: none of migrations 0001-0004 ever issue a `GRANT`. Every table in `public` had `TRUNCATE`/`REFERENCES`/`TRIGGER` granted to `anon`/`authenticated` but no `SELECT`/`INSERT`/`UPDATE`/`DELETE` at all — RLS policies were being enforced on privileges that didn't exist, so a fresh `supabase db reset` or a fresh hosted project provisioned outside the exact Supabase Cloud dashboard flow would have a completely non-functional app. Fixed in `0006_table_grants.sql`, which is why `join_household.sql`'s original 13 assertions needed no logic changes once role-switched — they were failing on missing grants, not on policy holes, until that migration landed.
5. ~~**Fix the `expenses_update` policy**~~ **Status: fixed, deliberately not as specified.** The review's proposed fix — adding `user_id = auth.uid()` to the policy's `with check` — was wrong and would have broken the product: Kapa is a shared pool where any member may legitimately edit any expense (`review-security.md`'s own threat model, actor (c)); that check would force every edit to re-attribute the row to the editor. Implemented instead as a `BEFORE UPDATE` trigger (`expenses_freeze_columns()` in `0007_expense_attribution.sql`) that makes `user_id`, `household_id`, `currency`, and `created_at` immutable to *any* UPDATE from *any* caller — including the row's own attributed user — while leaving amount/note/date/category freely editable by any co-member. One interaction the tests caught: the trigger initially also blocked the `ON DELETE SET NULL` FK action from item 3 (a real UPDATE as far as the trigger is concerned), so the immutability rule specifically permits transitions *to* `null` while still blocking re-attribution to any other specific user. `join_household`'s legitimate `household_id` move is exempted via a session-local bypass flag it sets around that one statement. Verified with 4 new pgTAP assertions in `expense_attribution.sql`: a co-member's attempts to rewrite `user_id`/`currency`/`household_id` all raise, while editing `amount_minor`/`note` still succeeds.
   - **Also fixed while touching this code**: `attribution.ts` returned `'you'`/`'partner'` as un-translated English literals regardless of locale (the docs-i18n review's finding). Fixed the same way `date.ts`'s `dayLabel` already does it — the lib function stays framework-agnostic and takes a `labels` object the caller builds from `t()`. Added `en`/`ru` keys in the `TodayList`/`HistoryList` namespaces, including the new `formerMember` label.
