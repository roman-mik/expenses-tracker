-- pgTAP RLS isolation suite for ledger_balance_snapshots (0016_ledger_balance_snapshots.sql).
...
insert into public.ledger_accounts (id, household_id, name, currency, type)
  values ('00000000-0000-0000-0000-0000000000a1', :'alice_household', 'Alice checking', 'RSD', 'personal') \gset

insert into public.ledger_balance_snapshots (household_id, account_id, balance_minor, expected_minor, currency)
...
select is(
  (select count(*)::int from public.ledger_balance_snapshots where household_id = :'alice_household'),
  1, 'alice sees her own balance snapshot');

select lives_ok(
  format($$ insert into public.ledger_balance_snapshots (household_id, account_id, balance_minor, expected_minor, currency)
...
with u as (
  update public.ledger_balance_snapshots set balance_minor = 1 where household_id = :'alice_household' returning 1
)
select is((select count(*)::int from u), 0, 'bob''s update of alice''s balance snapshots touches zero rows');

with d as (
  delete from public.ledger_balance_snapshots where household_id = :'alice_household' returning 1
)
select is((select count(*)::int from d), 0, 'bob''s delete of alice''s balance snapshots touches zero rows');

select lives_ok(
  format($$ insert into public.ledger_accounts (id, household_id, name, currency, type)
