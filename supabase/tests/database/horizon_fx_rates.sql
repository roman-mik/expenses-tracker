-- pgTAP RLS suite for horizon_fx_rates (0015_ledger_fx_rates.sql).
...
-- service_role cron.
insert into public.horizon_fx_rates (base_code, quote_code, rate_e8, as_of_date, source)
  values ('EUR', 'RSD', 11700000000, current_date, 'test-provider');

select tests.login_as(:'alice_id');

select is(
  (select count(*)::int from public.horizon_fx_rates where base_code = 'EUR' and quote_code = 'RSD'),
  1, 'an authenticated user can read fx rates');

select throws_ok(
  format($$ insert into public.horizon_fx_rates (base_code, quote_code, rate_e8, as_of_date, source)
...
-- 0015_horizon_fx_rates.sql) — table-level privilege is checked before RLS,
-- so this is a permission-denied error, not a silently-filtered 0-row update.
select throws_ok(
  format($$ update public.horizon_fx_rates set rate_e8 = 1 where base_code = 'EUR' and quote_code = 'RSD' $$),
  '42501', null,
