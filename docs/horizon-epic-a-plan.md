# Horizon Epic A — Accounts and current position

**Status:** completed, all 5 slices shipped (2026-08-19) · **Spec:** `docs/horizon-user-stories.md` §5 Epic A · **Plan of record:** `PLAN.md` §9

---

## 1. Context

`PLAN.md` §9 shipped the Horizon *shell* only — the `app/horizon/*` routes, the desktop gate,
`HorizonRail`, `AppSwitcher`, and seven placeholder screens. There is no horizon data model:
no `horizon_*` migrations exist, and every screen renders `HorizonPlaceholder`.
...
| Storage | Supabase, household-scoped `horizon_*` tables reusing `is_household_member()` (per `PLAN.md` §9) |
...
create table public.horizon_accounts (
  ...
  constraint horizon_accounts_currency_allowed check (currency in ('RSD','EUR','USD','RUB')),
  constraint horizon_accounts_type_allowed     check (type in ('business','personal','savings')),
  constraint horizon_accounts_name_len         check (char_length(btrim(name)) between 1 and 60)
);
create index horizon_accounts_household_idx on public.horizon_accounts (household_id, sort_order);
```
...
- **Reporting currency** —
  ```sql
  alter table public.households add column horizon_reporting_currency text;
  update public.households set horizon_reporting_currency = currency;
  alter table public.households
    alter column horizon_reporting_currency set not null,
    alter column horizon_reporting_currency set default 'RSD',
    add constraint households_horizon_reporting_currency_allowed
      check (horizon_reporting_currency in ('RSD','EUR','USD','RUB'));
  ```
...
```sql
create table public.horizon_fx_rates (
  ...
  constraint horizon_fx_rates_base_allowed  check (base_code  in ('RSD','EUR','USD','RUB')),
  constraint horizon_fx_rates_quote_allowed check (quote_code in ('RSD','EUR','USD','RUB')),
  constraint horizon_fx_rates_distinct      check (base_code <> quote_code)
);
```
...
```sql
create table public.horizon_balance_snapshots (
  id                  uuid primary key default gen_random_uuid(),
  household_id        uuid not null references public.households(id) on delete cascade,
...
Same RLS/grant treatment as `horizon_accounts`. The variance is `balance_minor - expected_minor`,
...
The repo's three-layer split (`lib/queries` → `lib/mutations` → `app/actions`) stays; horizon
code gets its own namespace under it, matching the `app/horizon/*` + `components/horizon/*`
convention that commit `bd3bc44` established.

```
apps/web/src/lib/horizon/
  types.ts            HorizonAccount, AccountType, FxRate, HorizonSettings, BalanceSnapshot
  mappers.ts          toHorizonAccount, toFxRate  (row → domain, same idiom as lib/mappers.ts)
...
apps/web/src/app/actions/
  horizon-accounts.ts  horizon-settings.ts  horizon-balances.ts
```
...
### `lib/horizon/fx.ts` — the one piece of real math
...
|---|---|---|
| `/horizon` (Today) | Hero total in reporting currency (sums only `include_in_total`); account chips showing native amount with the converted value secondary; a "rate is N days old" banner when stale; every converted figure reveals its source amount + the rate used, on hover/expand | A1, A2, A3 |
| `/horizon/accounts` **(new)** | Account list with add / edit / archive / reorder; name, currency, balance, type, `includeInTotal`; the reconcile panel (enter today's real balances → shows expected, actual, variance → writes snapshots) | A1, A4 |
| `/horizon/assumptions` | Reporting-currency selector; FX snapshot table (pair, rate, `asOfDate`, `source`, age badge, stale flag) | A2, A3 |

Adding `/horizon/accounts` is a **deliberate deviation** from the spec's seven-screen rail, with
...
`apps/web/src/components/horizon/HorizonRail.tsx` and extend `HorizonRail.test.tsx` to match.
...
**i18n:** add `Horizon.accounts.*`, `Horizon.today.*`, `Horizon.fx.*` and any new `Errors.*` keys to
**both** `apps/web/messages/en.json` and `ru.json` — `src/test/messages.test.ts` fails CI on key
drift. Each new page keeps its own `verifySession()` + `redirect('/login')`; the horizon layout
...
|---|---|---|
| Unit (node) | `lib/horizon/fx.test.ts` | Conversion across differing exponents (RSD 0 ↔ EUR 2) in both directions; rounding at the half; identity conversion; `pickRate` picks on-or-before, not latest; staleness at 29/30/31 days |
| Unit (node) | `lib/horizon/mutations/*.test.ts`, `queries/*.test.ts` | CRUD + household scoping, using `src/test/fake-supabase.ts` and `src/test/factories.ts` |
| Unit (node) | `app/actions/horizon-accounts.test.ts` | Unauthenticated → error; bad input → error, no DB call; success → `revalidatePath`. Mirror `app/actions/categories.test.ts` |
...
| jsdom | `AccountList.test.tsx`, `ReportingCurrencyPicker.test.tsx`, updated `HorizonRail.test.tsx` | Rendering, the new rail entry, form submit paths |
| pgTAP | `supabase/tests/database/horizon_accounts.sql` | Member reads/writes own household; **non-member reads zero rows and cannot insert**; `authenticated` cannot write `horizon_fx_rates` but can read it. Follow `supabase/tests/database/rls.sql` |
| Integration | `src/test/integration/horizon-accounts.itest.ts` | Currency/type check constraints reject bad values; `on delete cascade` from `households` |
...
**`feat/horizon-shell` is not merged yet**, so slice 1 waits on it landing. Conventional Commit
titles are enforced on both commits and PR titles.

| # | Branch / title | Contents | Status |
|---|---|---|---|
| 1 | `feat: add horizon accounts schema and reporting currency` | Migration 0014, `lib/horizon/{types,mappers,validation}`, queries/mutations/actions, `gen:types`, pgTAP + integration tests. No UI | ✅ Done (`c1ddd58`) |
| 2 | `feat: add horizon fx snapshots and daily refresh` | Migration 0015, `lib/horizon/fx.ts` + tests, `/api/fx-refresh`, `vercel.json` cron | ✅ Done (`b68ca54`) |
| 3 | `feat: show horizon accounts and totals` | `/horizon/accounts` screen, rail entry, Today hero + chips, i18n | ✅ Done (`350accf`) |
| 4 | `feat: add horizon fx and reporting currency settings` | Assumptions screen content | ✅ Done (`3368432`) |
| 5 | `feat: add horizon balance reconciliation` | A4 — migration 0016, reconcile panel | ✅ Done |

Update `PLAN.md` §9 as each slice lands (its "Not yet built" list is the horizon status board),
...
1. `pnpm dev`, sign in, open `/horizon` in a window **≥ 1024px** — below `lg` the desktop gate
...
6. `curl -H "Authorization: Bearer $CRON_SECRET" localhost:3000/api/fx-refresh` → 200 and rows
   in `horizon_fx_rates`. Re-run → same row count (idempotent). Without the header → 401.
...
8. A4: change one account's real balance in Studio, open the reconcile panel, confirm the
   variance is right and a `horizon_balance_snapshots` row is written.
