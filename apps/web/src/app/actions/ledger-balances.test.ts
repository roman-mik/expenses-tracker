...
import { reconcileLedgerBalancesAction } from './ledger-balances';
...
describe('reconcileLedgerBalancesAction', () => {
  it('rejects when signed out', async () => {
    mockedVerifySession.mockResolvedValue(null);
    expect(
      await reconcileLedgerBalancesAction({
        balances: [
          {
  ...
  it('rejects invalid schema input', async () => {
    ...
    const result = await reconcileLedgerBalancesAction({ balances: [] });
  ...
  it('reconciles balances on happy path', async () => {
    ...
    const result = await reconcileLedgerBalancesAction({
      balances: [
        {
          accountId: '00000000-0000-0000-0000-000000000001',
          balanceMinor: 1500,
