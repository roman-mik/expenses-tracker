...
describe('addLedgerAccount', () => {
  ...
  it('rejects invalid input', async () => {
    ...
    const result = await addLedgerAccount({ name: '', currency: 'XYZ' });
...
describe('editLedgerAccount', () => {
  it('reports a friendly error when the account is not found', async () => {
    mockedVerifySession.mockResolvedValue({ id: 'u1' });
    mockedGetHouseholdId.mockResolvedValue('h1');
    mockedCreateClient.mockResolvedValue(fakeSupabase().client);
    const result = await editLedgerAccount('missing', { archived: true });
  ...
  it('archives the account on the happy path', async () => {
    ...
    expect(await editLedgerAccount('a1', { archived: true })).toEqual({
      ok: true,
    });
  });
});
...
describe('moveLedgerAccount', () => {
  it('rejects when signed out', async () => {
    mockedVerifySession.mockResolvedValue(null);
    expect(await moveLedgerAccount('a1', 'up')).toEqual({
      ok: false,
      error: 'Not signed in.',
    });
  });
  ...
  it('succeeds even at a list edge (no-op reorder is still ok:true)', async () => {
    ...
    mockedCreateClient.mockResolvedValue(client);
    expect(await moveLedgerAccount('a1', 'up')).toEqual({ ok: true });
  });
});
