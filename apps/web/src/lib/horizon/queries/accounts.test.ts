...
import { getHorizonAccounts } from './accounts';

describe('getHorizonAccounts', () => {
  it('returns accounts for the household, ordered by sort_order', async () => {
    ...
    const accounts = await getHorizonAccounts(client, 'h1');
  ...
  it('returns an empty list when the household has no accounts', async () => {
    ...
    expect(await getHorizonAccounts(client, 'h1')).toEqual([]);
  });
});
