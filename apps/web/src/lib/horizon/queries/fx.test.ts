...
import { getHorizonFxRates } from './fx';

describe('getHorizonFxRates', () => {
  it('returns every snapshot, newest first, with no household scoping', async () => {
    const { client, db } = fakeSupabase();
    db.seed('horizon_fx_rates', [
      {
        base_code: 'EUR',
        quote_code: 'RSD',
    ...
    const rates = await getHorizonFxRates(client);
  ...
  it('returns an empty list when no snapshots exist yet', async () => {
    ...
    expect(await getHorizonFxRates(client)).toEqual([]);
  });
});
