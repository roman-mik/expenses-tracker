...
describe('createHorizonAccount', () => {
  it('appends after the current highest sort_order when none is given', async () => {
    ...
    const created = await createHorizonAccount(client, 'h1', {
      name: 'Business',
      currency: 'USD',
      type: 'business',
    });
...
describe('updateHorizonAccount', () => {
  it('returns null when the id is not in this household', async () => {
    ...
    expect(
      await updateHorizonAccount(client, 'h1', 'a1', { archived: true })
    ).toBeNull();
  });
  ...
  it('archives an account', async () => {
    ...
    const updated = await updateHorizonAccount(client, 'h1', 'a1', {
      archived: true,
    });
...
describe('moveHorizonAccount', () => {
  ...
  it('swaps sort_order with the previous sibling', async () => {
    const { client, db } = fakeSupabase();
    seedThree(db);
    expect(await moveHorizonAccount(client, 'h1', 'b', 'up')).toBe(true);
  ...
  it('is a no-op at the top of the list', async () => {
    const { client, db } = fakeSupabase();
    seedThree(db);
    expect(await moveHorizonAccount(client, 'h1', 'a', 'up')).toBe(false);
  });

  it('is a no-op at the bottom of the list', async () => {
    const { client, db } = fakeSupabase();
    seedThree(db);
    expect(await moveHorizonAccount(client, 'h1', 'c', 'down')).toBe(false);
  });

  it('returns false for an id not in this household', async () => {
    const { client, db } = fakeSupabase();
    seedThree(db);
    expect(await moveHorizonAccount(client, 'h1', 'missing', 'down')).toBe(
      false
    );
  });
});
