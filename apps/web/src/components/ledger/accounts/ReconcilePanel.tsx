...
import { reconcileLedgerBalancesAction } from '@/app/actions/ledger-balances';
...
export function ReconcilePanel({ accounts }: { accounts: LedgerAccount[] }) {
  ...
  const getMajorValue = (acc: LedgerAccount) => {
    return (
  ...
  const getNoteValue = (acc: LedgerAccount) => {
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await reconcileLedgerBalancesAction({ balances: entries });
      if (result.ok) {
