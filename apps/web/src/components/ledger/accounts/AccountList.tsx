...
import {
  ...
  editLedgerAccount,
  moveLedgerAccount,
} from '@/app/actions/ledger-accounts';
...
function AccountRow({
  ...
  const t = useTranslations('Ledger.accounts');
  ...
  const move = (direction: 'up' | 'down') => {
    ...
    startTransition(async () => {
      const result = await moveLedgerAccount(account.id, direction);
  ...
  const save = (values: AccountFormSubmitValues) => {
    setBusy('save');
    startTransition(async () => {
      const result = await editLedgerAccount(account.id, values);
      if (result.ok) {
        setEditing(false);
  ...
  const setArchived = (archived: boolean) => {
    ...
    startTransition(async () => {
      const result = await editLedgerAccount(account.id, { archived });
      if (result.ok) {
        setConfirming(false);
...
function AddAccountForm() {
  ...
  const submit = (values: AccountFormSubmitValues) => {
    startTransition(async () => {
      const result = await addLedgerAccount(values);
      if (result.ok) {
        setOpen(false);
...
export function AccountList({ accounts }: { accounts: LedgerAccount[] }) {
  const t = useTranslations('Ledger.accounts');
