...
import { ledgerAccount } from '@/test/factories';

const mockAdd = vi.fn();
const mockEdit = vi.fn();
const mockMove = vi.fn();
vi.mock('@/app/actions/ledger-accounts', () => ({
  addLedgerAccount: (...args: unknown[]) => mockAdd(...args),
  editLedgerAccount: (...args: unknown[]) => mockEdit(...args),
  moveLedgerAccount: (...args: unknown[]) => mockMove(...args),
}));
...
describe('AccountList', () => {
  ...
  it('lists active accounts, and puts archived ones under their own heading', () => {
    render(
      <AccountList
        accounts={[
          ledgerAccount({ id: 'a1', name: 'Checking' }),
          ledgerAccount({ id: 'a2', name: 'Old savings', archived: true }),
        ]}
      />
    );
  ...
  it('moves an account up via the action', async () => {
    ...
    render(
      <AccountList
        accounts={[
          ledgerAccount({ id: 'a1', name: 'First', sortOrder: 0 }),
          ledgerAccount({ id: 'a2', name: 'Second', sortOrder: 1 }),
        ]}
      />
    );
  ...
  it('shows an error toast when saving an edit fails', async () => {
    ...
    render(
      <AccountList accounts={[ledgerAccount({ id: 'a1', name: 'Checking' })]} />
    );
