...
describe('currentMonth', () => {
  it('returns YYYY-MM in the given timezone', () => {
    ...
    expect(currentMonth(new Date('2026-08-14T10:00:00Z'), 'Europe/Belgrade')).toBe(
      '2026-08'
    );
  });
  ...
  it('respects the timezone across the month boundary', () => {
    ...
    expect(currentMonth(new Date('2026-08-31T23:00:00Z'), 'Europe/Belgrade')).toBe(
      '2026-09'
    );
  ...
  it('zero-pads single-digit months', () => {
    expect(currentMonth(new Date('2026-01-15T12:00:00Z'), 'Europe/Belgrade')).toBe(
      '2026-01'
    );
  });
});
