...
export default async function Home() {
  ...
  // Timezone drives both the current-month window and "today" grouping.
  ...
  const [summary, expenses, categories] = await Promise.all([
    getSummary(supabase, user.id, month, now),
    listExpenses(supabase, user.id, { month }),
    ...
    <main className="flex-1 flex justify-center px-6 py-12">
      <div className="w-full max-w-xl flex flex-col gap-8">
        ...
        <section className="flex flex-col gap-3">
          ...
          <TodayList expenses={todays} categoryMap={categoryMap} />
        </section>
      </div>
    </main>
  );
