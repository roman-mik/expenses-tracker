  ...
  return (
    <div className="rounded-lg border border-accent/40 bg-accent/8 px-5 py-4 flex flex-col gap-2">
      <p className="text-sm text-ink/85">
        ...
        <strong className="text-accent-700">{formatMoney(overspend, currency)}</strong>{' '}
        this month. It happens — {daysLeft === 0 ? 'the' : `in ${daysLeft} `}
        {daysLeft === 0 ? 'reset' : daysLeft === 1 ? 'day the reset' : 'days the reset'}{' '}
        wipes the slate clean.
      </p>
      {suggestReducedCap ? (
        <p className="text-sm text-sage-700">
          Want to even out? Start next month at{' '}
          <strong className="text-ink">{formatMoney(recoveryCap, currency)}</strong> and
          you&rsquo;re right back on track — or keep your usual cap and enjoy the fresh
          start.
        </p>
      ) : (
        <p className="text-sm text-sage-700">
          No math to do — the 1st starts you fresh at your full cap. Just ease off
          till then.
        </p>
      )}
    </div>
